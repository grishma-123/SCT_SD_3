"""
Sudoku Solver — a desktop app built with CustomTkinter.

Features
--------
- 9x9 interactive grid with clear 3x3 box styling
- Type in any starting puzzle (digits 1-9), then hit Solve
- Backtracking algorithm fills in the rest of the grid
- "Given" numbers and "solved" numbers are shown in different colors
- Random Puzzle generator with Easy / Medium / Hard difficulty, plus Clear
- Basic validation: rejects duplicate numbers in a row/column/box and
...
CARD = "#1f2444"
FIELD = "#2b3159"
FIELD_ALT = "#333a6b"     # slightly lighter field, used to shade alternating 3x3 boxes
ACCENT = "#3ddad7"
ACCENT_DARK = "#28b8b5"
TEXT = "#f2f4ff"
SUBTEXT = "#9aa1c9"
...
# ----------------------------------------------------------------------
# Solver logic (plain backtracking with constraint checking)
# ----------------------------------------------------------------------
        ...
        for c in range(9):
            if board[r][c] == 0:
                ...
                if count < best_count:
                    ...
                    best = (r, c)
                    if best_count == 0:
                        return best  # dead end, bail out early
    return best
...
def solve_board(board):
    ...
    if cell is None:
        # Either solved, or a dead cell with no candidates was found.
...
def board_has_conflicts(board):
    """Check the *given* numbers for duplicate conflicts (ignoring zeros)."""
    for r in range(9):
        for c in range(9):
            num = board[r][c]
            if num == 0:
...
def generate_full_solution():
    """Generate a randomly-filled, fully solved 9x9 board."""
...
def generate_puzzle(clues=32):
    ...
    `clues` is roughly how many numbers remain visible."""
...
# Approximate clue-count ranges per difficulty (fewer clues = harder puzzle)
DIFFICULTY_CLUES = {
    "Easy": (40, 46),
    "Medium": (32, 38),
    "Hard": (24, 30),
...
class SudokuApp(ctk.CTk):
    def __init__(self):
        ...
        self.given_cells = set()  # cells the user typed in (vs solver-filled)
    ...
    def _build_grid(self):
        ...
        for box_r in range(3):
            for box_c in range(3):
                # shade alternating 3x3 boxes so the eye can separate them
        ...
        # --- difficulty side panel (right side) ---
    ...
    def _validate_digit(self, proposed):
        # Allow empty (so backspace works) or a single digit 1-9
        return proposed == "" or (len(proposed) == 1 and proposed in "123456789")

    def _on_key(self, row, col):
        # Any manual edit counts as a "given" number again, in accent-free text color
